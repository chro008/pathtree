(function (global) {
    ...
    SpaceTree.prototype.freshAllShowNodesState = function () {
        var thisobj = this,
            ...
            showNodeIds = dataLoader.getShowNodeIds(thisobj.clickNodeId),
        ...
        for (var i = 0, ch = drawNodes.length; i < ch; i++) {
            drawNodes[i].hasChildren = (dataLoader.getChildrenNodeIds(drawNodes[i].getId()).length > 0);
        ...
        if (clickid) {
            ...
            for (var i = 0, ch = relateItems.length; i < ch; i++) {
                ...
                if (item.type === "label") {
                    ...
                    if (animate) {
                        item.obj.animate({"fill": colorManager.getEdgeColor(selected)}, duration);
                    } else {
                        item.obj.attr("fill", colorManager.getEdgeColor(selected));
                    }
    ...
    SpaceTree.prototype.drawEachNode = function (node) {
        ...
        thisobj.drawEachNodeLabel(node, depth, pos);
    };
     ...
     * @param pos
     */
    SpaceTree.prototype.drawEachNodeLabel = function (node, depth, pos) {
        var thisobj = this,
            dataLoader = thisobj.dataLoader,
            animate = thisobj.options.animate,
            duration = thisobj.options.duration || 200;
        ...
        function setNodeLabel() {
            if (thisobj.options.setNodeLabelHtml) {
                var parentNode = thisobj.getNodeById(dataLoader.getParentNodeId(node.getId()));
                thisobj.options.setNodeLabelHtml(node, parentNode);
            } else {
            ...
            if (thisobj.options.addNodeLabelEventListener) {
                thisobj.options.addNodeLabelEventListener(node);
            } else {
    ...
    SpaceTree.prototype.drawEachEdge = function (node) {
        ...
        function setNodePortProperties() {
             ...
             */
            if (thisobj.options.getNodeEntranceProperties) {
                props = thisobj.options.getNodeEntranceProperties(thisNode, parentNode);
        ...
        if (!parentid) {
            ...
            var entrance = Util.extend(default_entrance, thisNode.entrance),                //入口小方块
            ...
            var selectState = (node.selected || Util.indexOf(clickNodeChildren, nodeid) >= 0),
